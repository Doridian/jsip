...
class TCPPkt extends IHdr {
	...
	static fromPacket(packet, offset, len, ipHdr) {
		...
		const data_offset = (bit.read(4) << 3) - 20;
		bit.skip(3);
	...
	_computeChecksum(ipHdr, packet, offset) {
		...
		let csum = computeChecksumPseudo(ipHdr, 6, fullLen);
	...
	toPacket(array, offset, ipHdr) {
		...
		packet[2] = (this.sport >>> 8) & 0xFF;
		packet[3] = this.sport & 0xFF;
		...
		packet[12] = ((data_offset >>> 3) << 4) + ((this.flags >>> 8) & 0x0F);
