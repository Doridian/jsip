...
function httpParse(datas) {
	...
	headerSplit.forEach((headerStr) => {
		...
		headers[headerStr.substr(0, colonPos).trim()] = headerStr.substr(colonPos + 1).trim();
	});
	...
	const statusCode = statusLine.substring(statusI + 1, statusI2);
...
function httpGet(url, cb) {
	const datas = [];
	const conn = tcpConnect(httpServerIp, 80, (data, tcpConn) => {
		// Data
		datas.push(data);
	}, (res) => {
		if (res === false) {
		...
		conn.send(stringToBuffer(`GET ${url} HTTP/1.1\r\nHost: doridian.net\r\nUser-Agent: jsip\r\nConnection: close\r\n\r\n`));
	}, () => {
		// Disconnect
		let res, err;
		try {
