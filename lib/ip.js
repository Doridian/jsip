...
class IPHdr extends IHdr {
	...
	static fromPacket(packet, offset) {
		...
		if (oLen > 20) {
			ipv4.options = new Uint8Array(packet.slice((bit.pos >>> 3) + offset, oLen - 20));
		} else {
			ipv4.options = new Uint8Array(0);
		}
	...
	toPacket(array, offset) {
		const packet = new Uint8Array(array, offset, this.options.length + 20);
