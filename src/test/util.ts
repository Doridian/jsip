...
export function decodeHexString(str: string) {
  ...
  for (let i = 0; i < str.length; i += 2) {
    res8[i / 2] = Number.parseInt(str.substr(i, 2), 16);
  }
  return res;
}
      ...
      switch (ipHdr.protocol) {
        case IPPROTO.ICMP:
          return {
            eth: ethHdr,
            ip: ipHdr,
            icmp: ICMPPkt.fromPacket(packet, offset, len),
          };
        case IPPROTO.UDP:
        ...
        case IPPROTO.TCP:
          return {
            eth: ethHdr,
            ip: ipHdr,
            tcp: TCPPkt.fromPacket(packet, offset, len, ipHdr),
          };
        default:
...
function compareIPOrMAC(
  a: IPAddr | MACAddr | undefined,
  b: IPAddr | MACAddr | undefined,
): boolean {
  if (!a || !b) {
    return !a && !b;
  }
