...
import { TCP_FLAGS, TCP_OPTIONS, TCPPkt } from "./index.js";
...
export class TCPConn extends EventEmitter {
  public static gotPacket(
    ...
    if (gotConn) {
      gotConn.gotPacket(ipHdr, tcpPkt);
      return;
    }
    ...
    rstTcpPkt.ackno = (tcpPkt.seqno + TCPConn.tcpSize(tcpPkt)) & 0xff_ff_ff_ff;
  ...
  private ack_sent = false;

  private wwnd = 0; // Max the remote end wants us to send
  private readonly rwnd = 65_535; // Max we want the remote end to send
  private rwnd_ackd = 65_535; // Max window ACK'd by the remote end
  ...
  private pbufferoffset = 0; // Offset of last sent element within buffer (for resend)
  private pbufferbytes = 0;

  private readonly rbuffer = new Map<number, RPacket>();

  public noDelay = true;
  private lastSend = 0;

  private sackSupported = false;
  ...
  public send(data: Uint8Array, zeroCopy = false) {
    if (!data.byteLength) {
      return;
    }
  ...
  private pushPBuffer(ip: IPHdr, tcp: TCPPkt) {
    ...
    this.seqno = (this.seqno + len) & 0xff_ff_ff_ff;
  ...
  private gotPacket(ipHdr: IPHdr, tcpPkt: TCPPkt) {
    ...
    if (!this.isInEitherWindow(tcpPkt.seqno)) {
      this.rejectPkt(
        ipHdr,
        tcpPkt,
        `Out of window (${this.calcWindowDiff(tcpPkt.seqno)} > ${Math.max(this.rwnd, this.rwnd_ackd)}`,
      );
      this.postHandlePacket(isEmptyPacket);
      return;
    }
  ...
  private sendSYN() {
    ...
    synPkt.setOption(
      TCP_OPTIONS.MSS,
      new Uint8Array([(this.mss >>> 8) & 0xff, this.mss & 0xff]),
    );
    const ip = this.makeIp(true);
  ...
  private postHandlePacket(onlyEmptyPackets: boolean) {
    ...
    if (!isNaN(this.ackno) && this.sackSupported && this.rbuffer.size > 0) {
      ...
      let sackRegionBegin = Number.NaN;
      let sackRegionEnd = Number.NaN;
        ...
        if (key === sackRegionEnd) {
          ...
          if (!isNaN(sackRegionBegin)) {
            ...
            if (sackRegions.length >= 3) {
              sackRegionBegin = Number.NaN;
              sackRegionEnd = Number.NaN;
              break;
            }
          }
          sackRegionBegin = key;
  ...
  private handlePacket(ipHdr: IPHdr, tcpPkt: TCPPkt) {
    ...
    this.ackno = (tcpPkt.seqno + len) & 0xff_ff_ff_ff;
  ...
  private processWPBuffer(needACK = false) {
  ...
  private processWBuffer(needACK: boolean) {
    ...
    while (
      avail > 0 &&
      (maxSend = Math.min(this.wwnd - this.pbufferbytes, this.mss)) > 0
    ) {
...
export function enableTCP(): void {
...
export function enableTCPEcho(): void {
