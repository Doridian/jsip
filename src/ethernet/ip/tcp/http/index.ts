...
class HttpCheckpointStream extends CheckpointStream<HttpParseState> {
  ...
  private readonly options: IHTTPOptionsFilled;
  private readonly method: string;
  private nextReadLen = 0;
  private readonly bodyChunks: Uint8Array[] = [];
  private readonly resolve: (res: IHTTPResult) => void;

  constructor(
    options: IHTTPOptionsFilled,
    resolve: (res: IHTTPResult) => void,
  ) {
  ...
  protected parseFunc(state?: HttpParseState) {
    switch (state) {
      case HttpParseState.StatusLine:
        ...
        this.statusCode = Number.parseInt(
          statusLine.substring(statusI + 1, statusI2),
          10,
        );
        if (!isFinite(this.statusCode) || this.statusCode <= 0) {
          throw new HttpInvalidErrror("Invalid response code in status line");
        }
        this.statusText = statusLine.slice(Math.max(0, statusI2 + 1));
      ...
      case HttpParseState.HeaderLine:
        ...
        if (curHeader.length > 0) {
          ...
          const headerKey = curHeader.slice(0, Math.max(0, colonPos)).trim();
          const headerValue = curHeader.slice(colonPos + 1).trim();
        ...
        const transferEncoding =
          this.headers.first("Transfer-Encoding") ||
        ...
        switch (transferEncoding.toLowerCase()) {
          ...
          case HttpTransferEncoding.Identity:
            ...
            this.nextReadLen = Number.parseInt(contentLengthStr, 10);
          ...
          default:
            throw new HttpInvalidErrror(
              `Invalid Transfer-Encoding: ${transferEncoding}`,
            );
        }
      ...
      case HttpParseState.BodyChunkLen:
        ...
        this.nextReadLen = Number.parseInt(this.readTrimmedLine(), 16);
      ...
      case HttpParseState.BodyChunkEnd:
        ...
        if (
          lineEnd.length > 2 ||
          (lineEnd.length === 2 && lineEnd[0] !== CHAR_CR)
        ) {
...
function httpPromise(
  ...
  tcpConn.once("connect", () => {
    ...
    tcpConn.send(
      new Uint8Array(stringToBuffer(`${data.join("\r\n")}\r\n\r\n`)),
      true,
    );
    if (body) {
      tcpConn.send(body);
  ...
  dnsResolveOrIp(url.hostname)
    .then((ip) => {
      tcpConn.connect(
        ip as IPAddr,
        url.port ? Number.parseInt(url.port, 10) : 80,
      );
    })
    .catch(reject);
}
...
export class HttpInvalidErrror extends Error {
  constructor(msg: string) {
    super(msg);
    this.name = "HttpInvalidErrror";
  }
}

export async function httpGet(options: IHTTPOptions): Promise<IHTTPResult> {
  ...
  return httpGetInternal(optionsFilled).then((result) => {
...
async function httpGetInternal(
  options: IHTTPOptionsFilled,
): Promise<IHTTPResult> {
