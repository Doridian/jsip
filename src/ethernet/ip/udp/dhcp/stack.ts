...
import { IP_BROADCAST, IP_NULL, IPAddr } from "../../address.js";
...
export class DHCPNegotiator {
  ...
  constructor(
    private readonly iface: IInterface,
  ...
  private handlePacket(dhcp: DHCPPkt) {
    ...
    switch (dhcp.options.get(DHCP_OPTION.MODE)![0]) {
      case DHCP_MODE.OFFER:
        logDebug(
          `${this.iface.getName()} Got DHCP offer, sending DHCP request...`,
        );
        sendIPPacket(
          this.makeDHCPIP(),
          this.makeDHCPRequestFromOffer(dhcp),
          this.iface,
        );
        break;
      case DHCP_MODE.ACK:
        ...
        subnet = subnetDHCP
          ...
          : IPNet.fromIPAndSubnet(ourIp, 32);
        ...
        if (routesRaw) {
          for (let i = 0; i < routesRaw.byteLength; i++) {
            ...
            fullIp.set(
              new Uint8Array(
                routesRaw.buffer,
                routesRaw.byteOffset + i,
                optLen,
              ),
            );

            const route = IPNet.fromIPAndSubnet(
              IPAddr.fromByteArray(fullIp, 0),
              subnetLen,
            );
            i += optLen;
        ...
        if (dnsServersRaw) {
          ...
          for (const server of dnsServers) addDNSServerFor(server, this.iface);
        }
        ...
        this.renewTimer = setTimeout(() => {
          this.renewInternal(ttl * 1000 - ttlHalftime);
        }, ttlHalftime);
      ...
      case DHCP_MODE.NACK:
        ...
        setTimeout(async () => this.negotiate(), 0);
        break;
    }
  }

  private async mkPromise(cb: VoidCB) {
    if (this.donePromise) {
      cb();
    } else {
  ...
  private negotiateInternal(secs = 0) {
    ...
    if (secs === 0) {
      this.xid = Math.floor(Math.random() * 0xff_ff_ff_ff) | 0;
      logDebug(
        `${this.iface.getName()} DHCP Initial XID ${(this.xid >>> 0).toString(16)}`,
      );
    } else {
  ...
  private renewInternal(renegotiateAfter = 0) {
    if (renegotiateAfter) {
      this.renewTimer = setTimeout(
        async () => this.negotiate(),
        renegotiateAfter,
      );
    }
    ...
    this.xid = Math.floor(Math.random() * 0xff_ff_ff_ff) | 0;
    logDebug(
      `${this.iface.getName()} DHCP Renew XID ${(this.xid >>> 0).toString(16)}`,
    );
    sendIPPacket(
      this.makeDHCPIP(true),
      this.makeDHCPRenewRequest(),
      this.iface,
    );
  }
  ...
  private makeDHCPIP(unicast = false) {
    const ip = new IPHdr();
...
export function addDHCP(
  iface: IInterface,
  metric: number = Metric.DHCPDefault,
): DHCPNegotiator {
  enableDHCP();
