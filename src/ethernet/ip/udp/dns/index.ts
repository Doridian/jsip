...
const DNS_SEG_PTR = 0b1100_0000;
const DNS_SEG_MAX = 0b0011_1111;
...
export class DNSPkt {
  ...
  public getFullLength() {
    ...
    for (const a of this.authority) {
      len += a.name.length + 2 + 10 + a.getDataLen();
    }
    for (const a of this.additional) {
      len += a.name.length + 2 + 10 + a.getDataLen();
    }
    return len;
  }
