...
export function removeRoute(route: IPRoute) {
  const idx = staticRoutes.findIndex(
    (value) =>
      routeEquals(route, value) && value.metric >= Metric.MinUserConfigurable,
  );
  ...
  for (const iface of getInterfaces()) {
    ...
    if (src) {
      routes.push({
        src,
        metric,
        iface: getLoopbackInterface(),
        subnet: IPNet.fromIPAndSubnet(src, 32),
      });
    }
  }
