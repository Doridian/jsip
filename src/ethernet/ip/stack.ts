  ...
  public static gotPacket(
    ...
    if (iface.isConfigured()) {
      if (!reversePathCheck(iface, ipHdr.saddr!)) {
        logDebug(
          `Reverse path check failed for src ${ipHdr.saddr} on ${iface.getName()}`,
        );
        return;
      }

      if (ipHdr.daddr!.isUnicast() && !iface.isLocalDest(ipHdr.daddr!)) {
        logDebug(
          `${iface.getName()} Discarding packet not meant for us, but for ${ipHdr.daddr}`,
        );
        return;
      }
    }
    ...
    if (!isFrag) {
      handlePacket(ipHdr, buffer, offset, iface);
      return;
    }
    ...
    if (curFrag.validUntil !== undefined && curFrag.last !== undefined) {
      ...
      let curPiece: IPFragment | undefined;
      ...
      if (gotAll) {
        const fullData = new ArrayBuffer(
          curFrag.fragments.get(curFrag.last)!.len + curFrag.last,
        );
        ...
        while (true) {
          ...
          for (const [i, element] of p8.entries()) {
            d8[curPiecePos + i] = element!;
          }
          if (!curPiece.ipHdr.mf) {
            break;
          }
        ...
        handlePacket(ipHdr, fullData, 0, iface);
      }
    }
  }
}
