...
export class MACAddr {
  public static fromString(macStr: string) {
    ...
    for (const [i, str] of macS.entries()) {
      mac.raw[i] = Number.parseInt(str, 16);
    }
    return mac;
  }
  ...
  public toString() {
    return (
      `${macPaddedOut(this.raw[0]!)}:${macPaddedOut(this.raw[1]!)}:${macPaddedOut(this.raw[2]!)}:` +
...
export const MAC_BROADCAST = MACAddr.fromByteArray([
  0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
]);
export const MAC_NULL = MACAddr.fromByteArray([
